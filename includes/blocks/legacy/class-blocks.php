<?php
/**
 * Block methods for the legacy blocks.
 *
 * @package user-profile-picture
 */

namespace MPP\Includes\Blocks\Legacy;
...
class Blocks {
	...
	public function load_gutenblock_svgs() {
		if ( '' !== get_post_type() ) {
			...
			$svg_icons = Functions::get_plugin_dir( '/img/social-logos.svg' );
	...
	public function add_gutenberg_scripts() {
		...
		wp_register_script( 'mpp_gutenberg', Functions::get_plugin_url( 'dist/blocks.build.js' ), array( 'wp-blocks', 'wp-element' ), Functions::get_plugin_version(), true );
		...
		wp_register_style( 'mpp_gutenberg', Functions::get_plugin_url( '/dist/blocks.editor.build.css' ), array(), Functions::get_plugin_version(), 'all' );
	}
	...
	public function add_gutenberg_styles() {
		...
		if ( has_block( 'mpp/user-profile' ) || has_block( 'mpp/user-profile-enhanced' ) ) {
			wp_enqueue_style( 'mpp_gutenberg', Functions::get_plugin_url( '/dist/blocks.style.build.css' ), array(), Functions::get_plugin_version(), 'all' );
			wp_enqueue_script( 'mpp_gutenberg_tabs', Functions::get_plugin_url( 'js/mpp-frontend.js' ), array( 'jquery' ), Functions::get_plugin_version(), true );
		}
	}
}
