...
gulp.task('pot', function () {
	...
	.pipe(sort())
	.pipe(wpPot({
		domain: 'metronet-profile-picture',
		destFile:'metronet-profile-picture.pot',
		package: 'User Profile Picture',
		bugReport: 'https://wordpress.org/plugins/metronet-profile-picture/',
...
gulp.task('gutenberg', function () {
	return gulp.src(webpackJSPaths)
		.pipe(plumber(reportError))
        .pipe(webpackStream({
			externals: {
				$: 'jQuery',
...
gulp.task('gutenbergmin', function () {
	return gulp.src(webpackJSPaths)
		.pipe(plumber(reportError))
		.pipe(webpackStream({
			externals: {
...
gulp.task('copy_for_zip', function () {
	return gulp.src('dist/**')
		.pipe(plumber(reportError))
		.pipe(gulp.dest('metronet-profile-picture'));
});

gulp.task('build_zip', function () {
	return gulp.src('metronet-profile-picture/**/*', { base: "." })
		.pipe(plumber(reportError))
		.pipe(zip('metronet-profile-picture.zip'))
		.pipe(gulp.dest('.'));
});

gulp.task('clean_zip', function () {
	return gulp.src('metronet-profile-picture', { read: false }).pipe(clean())
		.pipe(plumber(reportError));
});
...
gulp.task('watch', function () {
	...
	gulp.watch(webpackJSPaths, ['gutenberg', 'gutenbergmin']);
